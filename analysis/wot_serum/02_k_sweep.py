...
import csa_wot
from csa_wot import DEFAULT_H5AD, RESULTS_ROOT, jdump, load_serum, make_cfg, select_K_one
...
def parse_args():
    ...
    p.add_argument("--out", default=os.path.join(RESULTS_ROOT, "k_sweep"))
...
def main():
    ...
    data, _Z, _obs = load_serum(args.h5ad, day_min=args.day_min, day_max=args.day_max,
    ...
    rec["protocol"] = ("b: hold out an entire culture replicate, transfer the state "
                       "map by nearest expression prototype, score compression on the "
                       "held-out half; lambda_pm = 0 throughout (Degeneracy 3)")

    path = os.path.join(args.out, f"K_{K:03d}.json")
    jdump(rec, path)
